...
import { AppSettings, CategoryConfig, DEFAULT_CONFIG, DEFAULT_EXPORT_URL, DEFAULT_TODOIST_API_KEY, DEFAULT_TODOIST_TASK_ID } from "./types";
...
export async function loadSettings(): Promise<AppSettings> {
  try {
    const [configRaw, exportUrl, todoistApiKey, todoistTaskId] = await AsyncStorage.multiGet([CONFIG_KEY, EXPORT_URL_KEY, TODOIST_API_KEY_KEY, TODOIST_TASK_ID_KEY]);
    ...
    return { config: DEFAULT_CONFIG, exportUrl: DEFAULT_EXPORT_URL, todoistApiKey: DEFAULT_TODOIST_API_KEY, todoistTaskId: DEFAULT_TODOIST_TASK_ID };
  }
}
